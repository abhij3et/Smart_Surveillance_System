...
def weapon_detection():
    ...
    while True:
        ...
        with frame_lock: processed_frames['weapon'] = annotated 
        time.sleep(0.01)
...
def violence_detection():
    ...
    while True:
        ...
        if count%frame_rate==0:
            img=cv2.resize(annotated,(IMG_W,IMG_H)); img=np.expand_dims(img,axis=0)/255.0
            pred=violence_model.predict(img,verbose=0)[0][0]
            is_fight=pred>0.5; conf=pred if is_fight else (1-pred)
            if is_fight:
                now = time.time()
            ...
            cv2.putText(annotated,f"Prediction:{'fight' if is_fight else 'not_fight'}({conf:.2f})",(10,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
            if is_fight: cv2.putText(annotated,"ALERT: FIGHT DETECTED",(10,60),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
        with frame_lock: processed_frames['violence']=annotated
        time.sleep(0.01)

def generate_frames(stream_type):
    while True:
        with frame_lock: frame=processed_frames.get(stream_type)
        if frame is not None:
